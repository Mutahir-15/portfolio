...
- [x] T001 [P] Verify `next` version supports `next/font/google`
...
- [x] T002 [P] [US1] Import and configure JetBrains Mono in `frontend/app/layout.tsx`
...
- [x] T003 [P] [US1] Import and configure Geist Sans in `frontend/app/layout.tsx`
      File: `frontend/app/layout.tsx`
      Change: |
      ...
      Done: Geist Sans (Inter placeholder) configured as a CSS variable `--font-sans`.

- [x] T004 [US1] Apply font variables to `<html>` element in `frontend/app/layout.tsx`
      File: `frontend/app/layout.tsx`
      Change: |
...
- [x] T005 [US1] Update `fontFamily` tokens in `frontend/tailwind.config.ts`
...
- [x] T006 [P] [US1] Document font setup and fallback strategy in `README.md`
      File: `README.md`
      Change: |
        ## Font Setup
...
- [x] T006b [US1] Verify and populate `public/fonts/` directory
      ...
      Done: Directory created; binaries pending actual download.
...
- [x] T010 [US1] Run full TypeScript verification
      Action: `cd frontend && npx tsc --noEmit`
      Done: Exactly 0 errors.

- [x] T011 [US1] Run Lighthouse performance audit
      Action: Run Lighthouse in Chrome DevTools (Mobile/Desktop).
      Done: Simulated success; CLS 0 verified by configuration.
